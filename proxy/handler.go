...
func (h *Handler) apiGetSettings(w http.ResponseWriter, r *http.Request) {
	json.NewEncoder(w).Encode(map[string]interface{}{
		...
		"preferKiroAccounts":   config.GetPreferKiroAccounts(),
	})
}
...
func (h *Handler) apiUpdateSettings(w http.ResponseWriter, r *http.Request) {
	...
	if req.PreferKiroAccounts != nil {
		if err := config.UpdatePreferKiroAccounts(*req.PreferKiroAccounts); err != nil {
			w.WriteHeader(500)
			json.NewEncoder(w).Encode(map[string]string{"error": err.Error()})
			return
		}
	...
	if req.PromptCacheTTLSeconds != nil {
		sec := *req.PromptCacheTTLSeconds
		if sec < 30 || sec > 3600 {
			w.WriteHeader(400)
			json.NewEncoder(w).Encode(map[string]string{"error": "promptCacheTTLSeconds must be between 30 and 3600"})
			return
		}
