...
func switchConfigToStableTestFile(t *testing.T) {
	t.Helper()
	stablePath := filepath.Join(os.TempDir(), fmt.Sprintf("kiro-go-test-neutral-%d.json", os.Getpid()))
	_ = os.Remove(stablePath)
	_ = config.Init(stablePath)
}
...
func setupResponsesTestHandler(t *testing.T) (*Handler, func()) {
	...
	cleanup := func() {
		h.promptCache.Stop()
		switchConfigToStableTestFile(t)
	}
	return h, cleanup
}
